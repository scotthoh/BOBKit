...
NNbuild::NNbuild(int nfrag, clipper::Resolution resol,
		...
		double seq_rel, clipper::String newrestype, int cyc, int ncyc,
		 clipper::MiniMol mol_mr ,int freeflag,clipper::MiniMol  mol_wrk_in ,clipper::String ipfile,clipper::String ipcolfo,clipper::String ipcolfree,Ca_find cafind,double reso,double model_num, clipper::String threshold_select_method, bool nn_confirmation_cycles, int confirmation_cycles_num) {
	...
	for (int thrs = 0; thrs < thresholds.size(); ++thrs) {
		...
		runcyc buildcyc(nfrag, resol, mol_wrk_NN, knownstruc, xwrk, llktgt,
				...
				2, 3, 10,current_mol_log,cafind,residues_container,true); // 0,2 will do only one building cycle
		...
		if(nn_confirmation_cycles){
			...
			runcyc buildcycconf(nfrag, resol, mol_wrk_NN, knownstruc, xwrk, llktgt,
						...
						cycles_num, 1, 10,current_mol_log_confirmation,cafind,residues_container,true);
		}
