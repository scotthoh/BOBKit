// PyBind11 Python bindings for Clipper resolutionfn, basisfn, targetfn methods
// Copyright (C) 2016-2019 Tristan Croll, University of Cambridge
// chimerax-clipper/src/bindings/wrap_resol_fns.cpp
...
void declare_basisfn_base( py::module& m ) {
  py::class_<BasisFn_base, std::unique_ptr<BasisFn_base, py::nodelete>> basisfn_base(
      m, "_BasisFn_base" );
...
void declare_resolution_fn( py::module& m ) {
  py::class_<ResolutionFn>( m, "ResolutionFn" )
      ...
      */
      ...
      .def( py::init( []( const HKL_info& hkl_info, const BasisFn_base& basis_fn,
                          const TargetFn_base& target_fn, std::vector<ftype> params, ftype damp,
                          bool debug ) {
              return std::unique_ptr<ResolutionFn>(
                  new ResolutionFn( hkl_info, basis_fn, target_fn, params, damp, debug ) );
            } ),
            py::arg( "hkl_info" ), py::arg( "basis_fn" ), py::arg( "target_fn" ),
            py::arg( "params" ), py::arg( "damp" ) = 0.0, py::arg( "debug" ) = false )
  ...
  py::class_<ResolutionFn_nonlinear, ResolutionFn>( m, "ResolutionFn_nonlinear" )
      ...
      */
      ...
      .def( py::init( []( const HKL_info& hkl_info, const BasisFn_base& basis_fn,
                          const TargetFn_base& target_fn, std::vector<ftype> params, ftype damp,
                          bool debug ) {
              return std::unique_ptr<ResolutionFn_nonlinear>( new ResolutionFn_nonlinear(
...
void declare_basisfn_binner( py::module& m ) {
  ...
  basis_binner
      .def( py::init<const HKL_info&, const int&, const ftype>(), py::arg( "hkl_info" ),
            py::arg( "num_bins" ), py::arg( "power" ) = 1.0 )
      .def( py::init<const HKL_data_base&, const int&, const ftype>(), py::arg( "hkl_data" ),
            py::arg( "num_bins" ), py::arg( "power" ) = 1.0 )
