...
void declare_ccp4mtzfile(py::module &m) {
  ...
  using CCP4MTZcolumn_label_mode = CCP4MTZfile::CCP4MTZcolumn_label_mode;
  py::enum_<CCP4MTZcolumn_label_mode>(ccp4mtz, "CCP4MTZcolumn_label_mode")
      .value("Default", CCP4MTZcolumn_label_mode::Default)
      .value("Normal", CCP4MTZcolumn_label_mode::Normal)
      .value("Legacy", CCP4MTZcolumn_label_mode::Legacy);
  ccp4mtz.def(py::init<>())
      .def("open_read", &CCP4MTZfile::open_read)
      // .def("open_read", [](CCP4MTZfile& self, const std::string& fname) {
      // self.open_read(String(fname)); })
      .def("close_read", &CCP4MTZfile::close_read)
      .def("open_append", &CCP4MTZfile::open_append)
      // .def("open_append", [](CCP4MTZfile& self, const std::string& fname) {
      // self.open_append(String(fname)); })
      .def("close_append", &CCP4MTZfile::close_append)
      .def("open_write", &CCP4MTZfile::open_write)
      // .def("open_write", [](CCP4MTZfile& self, const std::string& fname) {
      // self.open_write(String(fname)); })
      .def("close_write", &CCP4MTZfile::close_write)
      .def("set_column_label_mode", &CCP4MTZfile::set_column_label_mode)
      ...
      .def("import_hkl_info",
           [](CCP4MTZfile &self, HKL_info &target) {
             self.import_hkl_info(target, false);
           })
      .def("import_crystal", &CCP4MTZfile::import_crystal)
      .def("import_dataset", &CCP4MTZfile::import_dataset)
      .def("import_hkl_data",
           (void(CCP4MTZfile::*)(HKL_data_base &, const String)) &
               CCP4MTZfile::import_hkl_data)
      // .def("import_hkl_data", [](CCP4MTZfile& self, HKL_data_base& cdata,
      // const std::string& mtzpath) { self.import_hkl_data(mtzpath); })
      ...
      .def("export_hkl_data",
           (void(CCP4MTZfile::*)(const HKL_data_base &, const String)) &
               CCP4MTZfile::export_hkl_data)

      .def("import_chkl_data", &CCP4MTZfile::import_chkl_data)
      .def("import_chkl_data",
           [](CCP4MTZfile &self, Container &target, const String &mtzpath) {
             self.import_chkl_data(target, mtzpath, "");
           })
      ...
      .def_property(
          "title", [](const CCP4MTZfile &self) { return self.title().c_str(); },
          [](CCP4MTZfile &self, const std::string &name) {
            self.set_title(String(name));
          })
      ...
      .def_property_readonly("ccp4_spacegroup_number",
                             &CCP4MTZfile::ccp4_spacegroup_number)
