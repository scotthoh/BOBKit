// Nanobind bindings for buccaneer-sequence
// Author: S.W.Hoh
// 2025 -
...
void declare_ca_sequence(nb::module_ &m) {
  ...
  casequence
      ...
      .def( "format", &Ca_sequence::format,
            ...
            "as string." )
      // need to test these
      .def_static( "phi_approx", &Ca_sequence::phi_approx, nb::arg( "z" ),
...
void declare_sequence_threaded(nb::module_ &m) {

  nb::class_<Sequence_threaded>(
      m, "Sequence_threaded",
      "Class with threads methods for sequencing for Ca groups.")
      .def(nb::init<>())
      .def(nb::init<const MiniMol &, const MMoleculeSequence &,
                    const double &>(),
           nb::arg("mol"), nb::arg("seq"), nb::arg("reliability"),
           "Constructor with model, sequence and sequence reliability.")
      .def("sequence", &Sequence_threaded::sequence, nb::arg("ichn"),
      ...
      .def("__call__", &Sequence_threaded::operator(), nb::arg("nthread") = 0,
           "Run single of multi-threaded.")
      .def("merge", &Sequence_threaded::merge, nb::arg("other"),
           "Merge results from multiple threads.")
