#include "buildkit_map_io.h"

#define DBG std:: << "[" << __FUNCTION__ << "] - "

// functions numpy arrays to nxmap/xmap
template <class T>
void buildkit::numpy_to_nxmap(pybind11::array_t<T, 3> data, const clipper::Cell &cell, clipper::NXmap<T> &nxmap)
{
  auto r = data.unchecked();
  clipper::Grid_sampling grid_sam(r.shape(0), r.shape(1), r.shape(2));
  clipper::Grid_range grid_map(clipper::Coord_grid(0, 0, 0), clipper::Coord_grid(r.shape(0) - 1, r.shape(1) - 1, r.shape(2) - 1));
...
template <class T>
void buildkit::numpy_to_xmap(pybind11::array_t<T, 3> data, const clipper::Cell &cell, clipper::Xmap<T> &xmap)
{
  auto r = data.unchecked();
  clipper::Grid_sampling grid_sam(r.shape(0), r.shape(1), r.shape(2));
  clipper::Grid_range grid_map(clipper::Coord_grid(0, 0, 0), clipper::Coord_grid(r.shape(0) - 1, r.shape(1) - 1, r.shape(2) - 1));
...
template <class T>
void declare_numpy_to_map(py::module &m)
{
