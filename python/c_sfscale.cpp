...
template <class T> void declare_sfscale_base(py::module &m, const char *dtype) {
  ...
  py::class_<Class, std::unique_ptr<Class, py::nodelete>>(m,
                                                          pyclass_name.c_str())
      .def("__call__", [](Class &self, HKL_data<F_sigF<T>> &fo,
                          HKL_data<F_phi<T>> &fc) { return self(fo, fc); })
      .def("__call__", [](Class &self, HKL_data<F_phi<T>> &fc,
                          HKL_data<F_sigF<T>> &fo) { return self(fc, fo); })
      .def("__call__",
