...
Target_fn_refine_amino_acid_fragment::Target_fn_refine_amino_acid_fragment( const clipper::Xmap<float>& xmap, const double& rot_step, const double& trn_step)
{
  xmap_ = &xmap;
  rot_step_ = rot_step;
  trn_step_ = trn_step;
  clipper::Map_stats mapstats(xmap);
  map_mean_ = mapstats.mean();
  map_std_ = mapstats.std_dev();
}

// methods for fragment refinement

//double Target_fn_refine_amino_acid_fragment::operator() ( const clipper::RTop_orth& rtop ) const
//{
//  // get map values accumulate as score
//  double score = 0.0;
//
//
//  return 
...
double Target_fn_refine_amino_acid_fragment::operator() ( const std::vector<double>& args ) const
{
  clipper::MMonomer target_res = tgt_res_;
  clipper::RTop_orth rtop = rtop_orth(args);
  target_res.transform(rtop);
  double score = 0.0;
  double zwt = 2.0;
  for ( int i = 0; i < target_res.size(); i++ )
  {
    score += zwt * ((xmap_->interp<clipper::Interp_cubic>(target_res[i].coord_orth().coord_frac(xmap_->cell())) - map_mean_) / map_std_);
  }
   
  return -score/float(target_res.size());
}

clipper::RTop_orth Target_fn_refine_amino_acid_fragment::rtop_orth( const std::vector<double>& args ) const
{
  //clipper::Euler_ccp4 euler(args[0], args[1], args[2]);
  clipper::Rotation rot(clipper::Euler_ccp4(args[0], args[1], args[2]));
  return clipper::RTop_orth( rot.matrix(), clipper::Coord_orth(args[3],args[4],args[5]) );
}

clipper::RTop_orth Target_fn_refine_amino_acid_fragment::refine( const clipper::MMonomer& res )
{ 
  tgt_res_ = res;
