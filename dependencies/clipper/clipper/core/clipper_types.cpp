...
String::String( const int i, const int w, const int p)
{ std::ostringstream s; s.width(w); s.precision(p); s << i; *this = s.str(); }

String::String( const long i, const int w, const int p)
{ std::ostringstream s; s.width(w); s.precision(p); s << i; *this = s.str(); }
